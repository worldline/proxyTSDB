...
class MetricSender(threading.Thread):
	...
	def __init__(self, queue, persistentQueueFileName, persistentQueueMaxElts, sleepTime=60):
		threading.Thread.__init__(self)
		...
		if persistentQueueFileName:
			self._diskQueue = dirq.QueueSimple.QueueSimple(persistentQueueFileName)
		...
		while self._alive:
			...
			try:
				try:
					...
					if curMetric: self._queue.appendleft(curMetric)
					raise e

				try:
					if self._diskQueue:
						#If MemoryQueueMetrics has sent, Send metrics in DiskQueue
			...
			if self._diskQueue:
				...
				while len(self._queue) > 0:
					metrics = []
					try:
						if DISK_MAX_SIZE > 0 and du(self._diskQueue.path) > DISK_MAX_SIZE*1024*1024:
...
class MetricSenderOpenTSDB(MetricSender):
	...
	def __init__(self, queue, persistentQueueFileName, persistentQueueMaxElts, host_port, sleepTime=60):
		MetricSender.__init__(self, queue, persistentQueueFileName, persistentQueueMaxElts, sleepTime)
	...
	def handle(self):
		try:
			while True:
				data = bytearray()
				while True:
					lim = (self.request.recv(1024, socket.MSG_PEEK).find(b"\n") + 1) or 1024
				...
				if re.match( r"put", data ) is not None:
					...
					result = "Status:\n"
					global METRIC_QUEUE
					if METRIC_QUEUE is not None: result += "RAM Queue elements : "+str(len(METRIC_QUEUE))+"\n"
					global DISK_METRIC_QUEUE
					if DISK_METRIC_QUEUE is not None: result += "DISK Queue elements : "+str(DISK_METRIC_QUEUE.count())+"\n"
...
class MetricServer(SocketServer.TCPServer):
	...
	def process_request(self, request, client_address):
		"""Start a new thread to process the request. And add it to the thread set"""
...
class MetricRequestHandler(SocketServer.BaseRequestHandler):
	...
	def handle(self):
		...
		if re.match( r"(GET|POST)", data) is not None:
			MetricHTTPRequestHandler(self.request, self.client_address, self.server)
		else:
			MetricTelnetRequestHandler(self.request, self.client_address, self.server)
		
def formatMetric(jsonMetric):
	"""parse a JSON metric and return a tuple of it if it is correctly formated"""
	...
	try:
		...
		raise Exception("Incomplete metric : {'metric': '*', 'timestamp': *, 'value': *}")

	if not re.match( METRICNAME_REGEX, jsonMetric['metric']):
		LOG.warning("Regex metric name")
		raise Exception("Metric name (sys|app|net|db).* : "+jsonMetric['metric'])

	return (jsonMetric['metric'], jsonMetric['timestamp'], jsonMetric['value'], jsonMetric['tags'])
...
def setup_logging(logfile=DEFAULT_LOG):
	...
	logHandler.setFormatter(logging.Formatter('%(asctime)s %(name)s[%(process)d] %(levelname)s: %(message)s'))
...
def parse_cmdline(argv):
	...
	parser.add_option('-D', '--daemonize', dest='daemonize', action='store_true', default=False, help='Run as a background daemon.')
	parser.add_option('-H', '--host', dest='host', default='localhost', metavar='HOST', help='Hostname to use to connect to the TSD.')
	parser.add_option('-P', '--pidfile', dest='pidfile', default='/var/run/proxyTSDB.pid', metavar='FILE', help='Write our pidfile')
	parser.add_option('--loglevel', dest='loglevel', type='str', default='INFO', help='Loglevel for output: <CRITICAL|ERROR|WARNING|INFO|DEBUG> (default INFO)')
	parser.add_option('--logfile', dest='logfile', type='str', default=DEFAULT_LOG, help='Filename where logs are written to.')
	parser.add_option('--diskqueuepath', dest='dirq_path', type='str', help='Path where to store the disk persistent queue')
	parser.add_option('--buffertype', dest='buffer_type', type='str', default='RAM', help='<DISK|RAM> (default RAM)')
	parser.add_option('--rammaxsize', dest='ram_max_size', type='int', default=512, help='in MB (0 for unlimited)(default 512)')
	parser.add_option('--diskmaxsize', dest='disk_max_size', type='int', default=1024, help='in MB (0 for unlimited)(default 1024)')
	parser.add_option('--sendperiod', dest='sleep_time', type='int', default=30, help='Second to wait between sends (default 30s)')
...
def main(argv):
	...
	try:
		...
		LOG.error("Unknown log level : "+options.logleve)
	if options.pidfile:
	...
	if 'DISK' != options.buffer_type: dirq_path = None
	sender = MetricSenderOpenTSDB(metricQueue, dirq_path, disk_buff_size, (options.host, 4242), sleep_time)
	global DISK_METRIC_QUEUE
